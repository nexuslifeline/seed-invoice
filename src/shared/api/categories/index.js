...
export const putCategory = (orgUuid, uuid, data, config) => {
  return apiClient.put(`organizations/${orgUuid}/categories/${uuid}`, data, config);
};

export const patchCategory = (orgUuid, uuid, data, config) => {
  return apiClient.patch(`organizations/${orgUuid}/categories/${uuid}`, data, config);
};

export const deleteCategory = (orgUuid, uuid, config) => {
  return apiClient.delete(`organizations/${orgUuid}/categories/${uuid}`, config);
};

export const getCategory = (orgUuid, uuid, config) => {
  return apiClient.get(u`organizations/${orgUuid}/categories/${uuid}`, config);
};
